...
```{r}
fisherClust_mps <- sapply(chrInfo$name[chrInfo$name != "MT"], function(chr){
  table(
    topTables$MPSvsWT$DE,
    topTables$MPSvsWT$chromosome == chr
  ) %>%
    fisher.test()
}, simplify = FALSE) %>%
...
```{r}
lapply(fisherClust_mps, function(fisherRes){
  fisherRes$p.value
}) %>%
  unlist() %>%
...
```{r}
regionsFwd_mps <- sapply(chrInfo$name[chrInfo$name != "MT"], function(chr){
...
```{r}
regionGenesFwd_mps <- sapply(regionsFwd_mps, function(region){
  subsetByOverlaps(genes, region)$gene_id
}, simplify = FALSE)
```

```{r}
clustRegionFwd_mps <- sapply(regionGenesFwd_mps, function(regionGenes){
  tt <- topTables$MPSvsWT %>%
    mutate(inRegion = gene_id %in% regionGenes)
  # Will throw an error if no genes in region
...
```{r}
sapply(clustRegionFwd_mps, function(fisherRes){
  if (length(fisherRes)) {
    fisherRes$p.value
  } else {
    return(1)
...
```{r}
genes_diversity <- readRDS(here("files/genes_diversity.Rds"))
```
...
# Localised enrichment

First split our genes into `r region_size / 1e6`Mbp regions.
Do this from both ends of the chromosome.

```{r}
# regionsFwd_mps <- 
# chrInfo %>%
#   dplyr::filter(name != "MT") %>%
#   with(setNames(length, paste0("chr", name))) %>%
#   lapply(function(length){
#     seq(from = 1, to = length, by = region_size) %>%
#       lapply(function(start){
#         IRanges(start = start, end = start + region_size - 1, width = region_size)
#       })
#   })

lapply(chrInfo$name[chrInfo$name != "MT"], function(chr){
  length <- chrInfo$length[chrInfo$name == chr]
  starts <- seq(1, length, by = region_size)
  lapply(starts, function(start){
    genes %>%
