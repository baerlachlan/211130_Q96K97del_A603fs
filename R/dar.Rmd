...
metadata <- read_csv(here("files/samples.csv")) %>%
  mutate(
    genotype = case_when(
      ...
      genotype == "MPS-IIIB" ~ "MPSIIIB",
    ),
    genotype = factor(genotype, levels = c("WT", "EOfAD", "MPSIIIB")),
    group = genotype
  ) %>%
  dplyr::arrange(genotype) %>%
  dplyr::select(sample, genotype, everything())
```
...
```{r cumDAR_MPSIIIBvsWT}
plotCumDAR(DAR$MPSIIIBvsWT, "24", "MPSIIIB vs. WT")
```
...
```{r winRanges_MPSIIIBvsWT}
plotWinRanges(winRanges$MPSIIIBvsWT, "MPSIIIB vs WT")
```
...
plotChrDAR(
  DAR = DAR$EOfADvsWT,
  chromosome = "17",
  features = genes[genes$gene_id %in% deGenes$EOfADvsWT],
...
```{r DAR_MPSIIIBvsWT_mutant}
plotChrDAR(
  DAR = DAR$MPSIIIBvsWT,
  chromosome = "24",
  features = genes[genes$gene_id %in% deGenes$MPSIIIBvsWT],
  foi = genes[genes$gene_name == "psen1"]
)
```
