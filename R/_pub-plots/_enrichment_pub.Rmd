...
map2(ranks_d, ranks_d_dar, function(before, after){
  ...
  left_join(before, after) %>%
    ...
    dplyr::arrange(seqnames == 24) %>% # plot chr24 on top
    ggplot(aes(rank_before, rank_after)) +
    geom_point(
      aes(colour = seqnames == 24, alpha = seqnames == 24),
      size = 0.5
    ) +
    ...
    scale_alpha_manual(values = c(1, 0.4), breaks = c(TRUE, FALSE)) +
    guides(alpha = "none")
})
```
...
```{r}
ggsave(
  "~/workspace/211130_Q96K97del_A603fs/effects_of_weighting.png",
